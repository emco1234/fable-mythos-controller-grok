...
description: |
  ...
  the deterministic Python controller at ~/.grok/skills/fable-mythos-controller/scripts/run-controller.sh.
...
4. **Invoke the controller** via the helper script. The script handles
   ...
   ```bash
   bash ~/.grok/skills/fable-mythos-controller/scripts/run-controller.sh \
...
- Controller adapters that spawn real Grok agents require an active
  Grok login. Without login, the adapter writes a STUB transcript and
