# fable-mythos-controller-grok

**Walking Skeleton** — concept validation for a deterministic Python asyncio controller that drives the Reliability Harness v2 inside [Grok Build CLI](https://x.ai/cli).

> ⚠️ **NOT production-ready.** This is a concept-validation scaffold to test whether a code-based controller is preferable to the prompt-only harness in [`emco1234/fable-mythos-grok`](https://github.com/emco1234/fable-mythos-grok). The existing `fable-mythos-grok` repo is unchanged on `main` and continues to work via pure prompting.
...
| Grok CLI adapter (stub) | ✅ Implemented |
...
The original three (`fable-mythos-grok/opencode/zcode`) work via prompt-only sub-agents. This controller replaces prompt-based orchestration with a Python DAG scheduler — a fundamentally different architecture. The new repos are isolated for testing so the prompt-based harnesses keep working.
...
2. Port to `fable-mythos-grok` as a new major version
3. Mirror to OpenCode + ZCode adapters
...
├── adapters/
│   └── grok_adapter.py                 # Grok CLI spawn adapter (stub)
├── examples/
...
- Spawn real Grok subagents (the adapter is a stub)
...
- [`emco1234/fable-mythos-grok`](https://github.com/emco1234/fable-mythos-grok) — Prompt-based harness (production-ready, unchanged)
